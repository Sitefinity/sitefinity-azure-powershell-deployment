...
function CreatePackage($serviceDefinitionPath, $outDir, $projectLocation, $roleName, $rolePropertiesFilePath)
{
	...
	LogMessage ("[Start] creating azure cloud package with the following properties: definition path {0}, out directory {1}, role {2}, sites {3}, role properties {4}" -f $serviceDefinitionPath, $out, $role, $sites, $roleProperties)
